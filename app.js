let jokesFavouriteColection = JSON.parse(
  localStorage.getItem('jokesFavouriteColection'),
);
...
function randerCategory(categoriesArray) {
  ...
  categoriesList.forEach((item) =>
    categoriesContainer.insertAdjacentElement('beforeend', item),
  );
}
...
//разделить создание элемента и рендера и посмотреть остальные функции можно ли их так разделить
function createJokeCard(item, container, functionFromBtn, likeBtnSrc) {
  const jokeCard = document.createElement('div');
  jokeCard.classList.add('joke-card');
  jokeCard.innerHTML = `<div class = "box-card-icon"><img src="images/message.png" alt="icon"></div>
  ...
  update.innerHTML = `Last update: <b>${getTimeLastUpdate(
    item.updated_at,
  )} hours ago</b>`;
...
function randerFavourite(jokeArr) {
  jokeArr.forEach((item) =>
    createJokeCard(
      item,
      jokeFavouriteConteiner,
      removeJokeFromFavourite,
      srcImgLike.ImgLikeActive,
    ),
  );
}
...
function saveToMemory() {
  localStorage.setItem(
    'jokesFavouriteColection',
    JSON.stringify(jokesFavouriteColection),
  );
}
